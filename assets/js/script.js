...
$('#verifyStudent').on('click', function () {
	...
	if (certiNum != "") {
		...
		$.ajax({
			...
			success: function (response) {
                $(".loader").hide();
				if (response.indexOf("Undefined variable") <= -1 && response.indexOf("undefined") <= -1 && response.indexOf('[""]')) {
					...
					$(".dispBatchNum").text(obj[1].batch);
					$(".dispCollegeName").text(obj[1].college);
					...
					if(obj[1].certificateNumber.indexOf("CO-")>=0){
						$(".modalResultDiv").hide();
						$(".modalCoordinatorResultDiv").show();
					}
					else{
						$(".modalResultDiv").show();
						$(".modalCoordinatorResultDiv").hide();
					}
